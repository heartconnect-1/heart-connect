export const VERSION='hc-admin-auth-v1';
...
function setCookie(token,maxAge){
  ...
  const age=Math.max(300,Math.min(SESSION_MAX_AGE,Number(maxAge)||SESSION_MAX_AGE));
  return COOKIE+'='+safe+'; Path=/; Secure; HttpOnly; SameSite=Strict; Max-Age='+age+'; Priority=High';
}
...
async function signIn(request,env){
  ...
  try{
    ...
    const maxAge=Math.max(300,Math.min(SESSION_MAX_AGE,Number(d.expires_in||SESSION_MAX_AGE)-30));
    return json({ok:true,role,sessionMaxAgeSeconds:maxAge},200,{'set-cookie':setCookie(d.access_token,maxAge)});
  }catch(e){
