...
function edge(response){const secured=securityHeaders(response);const headers=new Headers(secured.headers);headers.set('x-heart-connect-edge',EDGE_VERSION);headers.set('x-heart-connect-native-version',NATIVE_VERSION);headers.set('x-heart-connect-media-version',MEDIA_VERSION);headers.set('x-heart-connect-firewall-version',FIREWALL_VERSION);headers.set('x-heart-connect-performance-version',PERFORMANCE_VERSION);return new Response(secured.body,{status:secured.status,statusText:secured.statusText,headers})}
